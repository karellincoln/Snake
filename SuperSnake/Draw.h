...
public:
	...
	void drawWhichPictrue(pos x, pos y, short int width, short int height,
		short int typ, short int color = VGA_WHITE);
	
	//分别在左上为（x0,y0),和右下为（x1,y1)的填满和空的矩形。
	void drawARect(pos x0, pos y0, pos x1, pos y1, short int color = VGA_WHITE);
	void fillARect(pos x0, pos y0, pos x1, pos y1, short int color = VGA_WHITE);
	
	//这两个是画正方形。
	void drawASquare(pos x, pos y, short int size, short int color = VGA_WHITE);
	void fillASquare(pos x, pos y, short int size, short int color = VGA_WHITE);
...
private:
	...
	void drawAPicture(pos x, pos y, short int width, short int height,
					  char tp[], short int color = VGA_WHITE);
